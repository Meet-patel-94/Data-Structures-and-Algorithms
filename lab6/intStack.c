...
{
if ( top == 100 ) {
		fprintf(stderr, "Overflow error! Stack is full!\n");
	}

	else	{
  		stack[top] = thing2push;
		top = top + 1; //increment top since you successfully pop it
	}
}
...
{
	if (top == 0)
		return 1; //1 for true, 0 for false
	else
  		return 0;
}
...
{
	if ( isEmpty() ) {
		fprintf(stderr, "Underflow error! Stack is empty!\n");
		return -1;
	}

	else	{
        	top = top - 1; //decrement top since you successfully pop it
   		return (stack[top]);
	}
}
